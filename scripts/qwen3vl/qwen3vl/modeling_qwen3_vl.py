...
class Qwen3VLVisionPatchMerger(nn.Module):
    def __init__(self, config: Qwen3VLVisionConfig, use_postshuffle_norm=False) -> None:
        ...
        # Change 'norm' to 'ln_q' to match checkpoint
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Change 'self.norm' to 'self.ln_q'
        x = self.ln_q(x.view(-1, self.hidden_size) if self.use_postshuffle_norm else x).view(
            -1, self.hidden_size
        )
        ...
        if inputs_embeds is not None:
            vision_start_mask = (
                ...
                == self.get_input_embeddings()(
                    torch.tensor(
                        vision_start_token_id, dtype=torch.long, device=inputs_embeds.device
                    )
                )
            )[..., 0]
            image_mask = (
